...
- Its full text is freely retrievable but is not under an explicit open-reuse license —
  treat it as a citable authority, not a source to copy wholesale into this MIT-licensed
  repo.
...
in near-universal use across manufacturers and predate/postdate any single manual in this
repo — expect to see them without expansion.
...
Every manual in this repo went through OCR on a scanned/photocopied source, so these
patterns are worth knowing before treating an odd-looking value as an error in the
original rather than an artifact of the scan. From a peer-reviewed study of OCR error
behavior on degraded documents ([Lopresti, Lehigh University, IJDAR](https://www.cse.lehigh.edu/~lopresti/tmp/AND08journal.pdf)):

- **Ω (ohm) → "2"** — already documented per-manual in each `llm-instructions.md`; the
  most common symbol-specific misread in this domain.
...
None of this overrides the manual's own `10-needs-review.md` flags — it's context for
*why* an unflagged value might still look visually odd, not license to silently
"correct" anything yourself.

## What wasn't found / open gaps

Be aware of what this file does *not* cover, rather than assuming silence means it
doesn't matter:

- **Diagnostic flowchart notation** (symbols/conventions used in troubleshooting
  flowcharts) — not researched in this pass. If a manual's flowchart uses a symbol or
  convention that isn't self-explanatory, say so rather than guessing at a standard
  meaning.
- **DTC second-digit convention** (generic/core vs. manufacturer-specific) — see the
  ...
  15031-6 text.
- None of the verified torque-table/DTC examples above were pulled from the actual
  Toyota 4A-F/4A-GE manual in this repo — they're cross-corroborated from other Toyota
  models (FJ Cruiser, Corolla, RAV4/Camry). The pattern is very likely to hold, but
  hasn't been directly confirmed against this specific PDF.

## Existing resources evaluated (why this file exists instead of reusing one)

Before writing this file, existing options were checked for reuse:

- **ASE Glossary of Automotive Terminology** ([PDF](https://www.asepractice.com/PDF/ASE_Glossary.pdf)) — turned out to be a bilingual
  English/Spanish **term-translation list** (e.g. "backfire" → "contra explosión"), not a
  definitional glossary, and carries no reuse license (ASE's own site: "not to be reused
  without express written consent"). Not usable as a source to copy from.
- **Wikipedia — Glossary of automotive terms** ([link](https://en.wikipedia.org/wiki/Glossary_of_automotive_terms)) — CC BY-SA 4.0. Reusable
  with attribution + share-alike, but not a clean drop-in for an MIT-licensed repo without
  triggering those obligations — used here as a reference to write original definitions
  from, not copied verbatim.
- **OBDII.DTC** ([GitHub, lennykean/OBDII.DTC](https://github.com/lennykean/OBDII.DTC)) — genuinely MIT-licensed, a reusable
  ...
  by the P/C/B/U prefix convention. Worth adopting directly if this project ever wants a
  full DTC lookup table rather than just documenting the convention — not pulled in here
  since no manual in this repo currently needs a full DTC database.
- **Existing Claude Skills / GPTs for automotive work** — an "Automotive Expert" Claude
  ...
  software, fleet/telematics, ADAS, and functional-safety compliance standards (ISO
  26262, ISO/SAE 21434, Automotive SPICE) — not reading OEM repair manuals. GPT Store has
  several consumer-facing "Auto Mechanic GPT" chatbots, but they're closed/proprietary
  with nothing to inspect or reuse. Nothing found fits this repo's actual need, which is
  why this file exists as original material.
...
- [ASE Glossary of Automotive Terminology](https://www.asepractice.com/PDF/ASE_Glossary.pdf) — evaluated, not reused (see above)
