...
python scripts/build_manual_index.py manuals/<slug>/   # data/manual-index.jsonl — the fast path
python scripts/09_link_index.py                       # all-files.md + repo-root MANUALS.md
```

`build_manual_index.py` flattens every spec **table** AND every prose spec statement (torque
