...
apt/brew installs, **not** pip packages. A bare chat assistant (ChatGPT web without
tools, claude.ai without artifacts/code, Gemini chat) has no persistent shell,
filesystem, or network to install them, so it **cannot OCR a scan or emit a PDF**. This
is a system-dependency limit, not a Python limit — rewriting in another language
wouldn't remove it.
...
## Partial path — ChatGPT Advanced Data Analysis / hosted code tools

Some Python sandboxes can run PyMuPDF and may have OCR binaries available, so **small**
manuals can sometimes go end to end. Limits vary by product: missing system binaries, no
network, file-size caps, and runtime caps are common. Fine for a short manual; unreliable
for a full one.
